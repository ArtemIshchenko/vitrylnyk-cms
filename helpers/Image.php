...
class Image
{
    ...
    static function copyResizedImage($inputFile, $outputFile, $width, $height = null, $crop = true)
    {
		...
		public static function copyFile($fileName, $destinationDir, $path='/media/img/source')
    {
			 $sourcePath = Yii::getAlias('@app').$path;
        if(!FileHelper::createDirectory($sourcePath)){
            throw new HttpException(500, 'Cannot create "'.$sourcePath.'". Please check write permissions.');
        }
				$sourceFileName = $sourcePath . DIRECTORY_SEPARATOR . $fileName;
				$destinationFileName = Upload::getUploadPath($destinationDir) . DIRECTORY_SEPARATOR . $fileName;
				$link = str_replace('\\', '/', str_replace(Yii::getAlias('@webroot'), '', $destinationFileName));
				$res = null;
				if(file_exists($sourceFileName))
					$res = copy($sourceFileName, $destinationFileName);
				return $res ? $link : '';
    }
}