...
use app\modules\catalog\models\ItemData;
...
/**
 * Catalog module API
 * @package app\modules\catalog\api
 *
...
class Catalog extends \app\components\API
{
    private $_cats;
    private $_items;
    private $_adp;
    ...
    public function api_cat($id_slug)
    {
		...
		public function api_table($id_slug)
    {
        if(!isset($this->_cats[$id_slug])) {
            $this->_cats[$id_slug] = $this->findCategory($id_slug);
        }
				$result = [];
				foreach($this->_cats[$id_slug]->items() as $item) {
					foreach($item->data as $k => $value) {
						$result[$item->title][$k] = $value;
					}
				}
        return $result;
    }
